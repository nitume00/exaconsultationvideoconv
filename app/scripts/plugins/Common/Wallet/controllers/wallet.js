...
/**
 * @ngdoc function
 * @name abs.Wallet.controller:WalletController
 * @description
 * # WalletController
 * Controller of the abs.Wallet
 */
angular.module('abs.Common.Wallet')
    .controller('WalletController', function($rootScope, $scope, $window, Country, wallet, flash, $location, $filter, $state, paymentGateways, ConstPaymentGateways, UserMeFactory) {
                ...
                if (payment_response.error.code === 0) {
					if (payment_response.zazpay !== undefined) {
						...
						angular.forEach($scope.form_fields_tpls, function(key, value) {
							if (value === 'buyer') {
								$scope.form_fields[value] = 'scripts/plugins/Wallet/views/default/buyer.html';
							}
							if (value === 'credit_card') {
								$scope.form_fields[value] = 'scripts/plugins/Wallet/views/default/credit_card.html';
							}
							if (value === 'manual') {
								$scope.form_fields[value] = 'scripts/plugins/Wallet/views/default/manual.html';
							}
